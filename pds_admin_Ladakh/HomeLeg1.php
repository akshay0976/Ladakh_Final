	...
	if (checkbox.checked) {
		...
		fetch(pythonUrl + 'getGraphDataleg1', {
			...
			.then(data => {
				...
				var totalDemandFRice = Object.values(data.District_Demand_Rice).reduce((acc, demand) => acc + demand, 0);

				var totalDemand = totalDemand + totalDemandRice + totalDemandFRice;
				...
				if (totalCapacity > 0 && totalDemand > 0) {
					if (totalCapacity >= totalDemand) {
						...
						document.getElementById("result").innerHTML = "<span style='font-weight: bold; font-size: 20px; color: red;'>Optimisation cannot be done infeasible solution.</span>";
